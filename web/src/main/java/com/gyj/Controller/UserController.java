...
import com.gyj.entity.QUserInfo;
...
public class UserController {
    ...
    //接口还没有调通，可能需要安装插件
    @ApiOperation(value = "利用predicate分页查询列表")
    @RequestMapping(value = "/getPredicatePageList", method = RequestMethod.GET)
    public Page<UserInfo> getPredicatePageList(@QuerydslPredicate(root = QUserInfo.class) Predicate predicate,
                                               @ApiParam Pageable pageable) throws Exception {

        Page<UserInfo> pageList = userService.findPredicateByPage(predicate, pageable);
        return pageList;
    }
    ...
    /**
     * 根据用户名获取搜索记录，目前还不能验证从token里拿到登录用户名
     * @param pageable
     * @return
     */
    @ApiOperation(value = "获取用户搜索记录列表", notes = "获取用户搜索记录列表")
    @RequestMapping(value = "/searchHistorys", method = RequestMethod.GET)
    public Page<String> searchHistorys(@ApiParam Pageable pageable) {
        String token = TokenUtil.getToken();
        return searchService.getPageSearch(token, pageable);
    }
